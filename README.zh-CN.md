...
- **原始 TCP & UDP 摄取**，数据端口可配置（默认 `45678`）。
...
- **HTTP 摄入** —— 通过 `POST /api/ingest` 写入日志，AI 代理（仅需 HTTP 工具）也能推送日志；
  详见 [skills](skills) 目录。
- **鉴权** —— 登录页保护界面，所有 API 请求需在 `Authorization` 头中携带访问密钥
  （默认 `tm-76c296d3244f05b26cd082254`，可在设置中或通过 `--key` 修改）。
...
| `--key` | *(设置)* | 超级管理员登录密钥，与数据库密钥同等效力，均可用于登录与 API 请求。 |
